...
const createCategoryAndReturn = `-- name: CreateCategoryAndReturn :execresult
INSERT INTO categories (id, name, description) VALUES (?, ?, ?)
`

type CreateCategoryAndReturnParams struct {
	ID          string
	Name        string
	Description sql.NullString
}

func (q *Queries) CreateCategoryAndReturn(ctx context.Context, arg CreateCategoryAndReturnParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, createCategoryAndReturn, arg.ID, arg.Name, arg.Description)
}
...
SELECT c.id, c.category_id, c.name, c.description, c.price, ca.name as category FROM courses c join categories ca on c.category_id = ca.id
`
